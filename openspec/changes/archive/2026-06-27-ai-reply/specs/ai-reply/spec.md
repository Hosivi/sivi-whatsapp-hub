...
- `ANTHROPIC_API_KEY` (required): the key passed to `createAnthropicAdapter`.
  If absent, the Anthropic adapter MUST NOT be instantiated and the loop MUST be
  effectively disabled (treated as no config).
- `AI_MODEL` (optional): the model string passed to `createAnthropicAdapter`.
  Default MUST be `claude-haiku-4-5` if the variable is absent.

#### Scenario: AI_MODEL absent → default claude-haiku-4-5

- GIVEN `AI_MODEL` is not set in the environment
- WHEN the Anthropic adapter is instantiated
- THEN the adapter uses model `claude-haiku-4-5`

#### Scenario: AI_MODEL set → that model is used

- GIVEN `AI_MODEL=claude-opus-4-5` is set
- WHEN the Anthropic adapter is instantiated
- THEN the adapter uses model `claude-opus-4-5`
